...
for i in range(0, pkt_count):
    ...
    # construct the UDP datagram using the scapy provided UDP object
    datagram = UDP(sport=src_port, dport=dest_port)
    # use the overriden div operator to format the packet for sending
    pkt = packet / datagram
    # send the UDP packet
    send(pkt)
